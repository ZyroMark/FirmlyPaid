...
namespace FirmlyPaid.Gateway.Tests;

/// <summary>
/// Step 1 checkpoint: the service starts with its configuration validated and answers
/// /health with its own name.
/// </summary>
public class HealthEndpointTests : IClassFixture<WebApplicationFactory<FirmlyPaid.Gateway.ApiMarker>>
{
    private readonly WebApplicationFactory<FirmlyPaid.Gateway.ApiMarker> _factory;

    public HealthEndpointTests(WebApplicationFactory<FirmlyPaid.Gateway.ApiMarker> factory) => _factory = factory;
    ...
    public async Task Health_ReportsHealthyAndNamesTheService()
    {
        ...
        document.RootElement.GetProperty("service").GetString().Should().Be("FirmlyPaid.Gateway");
    }
    ...
    public async Task OpenApiDocument_IsPublished()
    {
        ...
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        (await response.Content.ReadAsStringAsync()).Should().Contain("FirmlyPaid.Gateway");
    }
}
